...
//	Skip events that do not have a primary vertex. the small value '1e-5' is ok for hard coding.
	...
	if((pow(muEvent->primaryVertexPosition().x(),2)+pow(muEvent->primaryVertexPosition().y(),2)) > 4.0) return kStOK ;
...
//	select mb and central trigger
	 ...
	 if((!muEvent->triggerIdCollection().nominal().isTrigger(450005))&&
	    ...
	    (!muEvent->triggerIdCollection().nominal().isTrigger(450025)))return kStOK ;
...
//		StPhysicalHelixD   helix        = track->helix();//inner helix. good for dca to PV.
		...
		if(!dcaG){cout<<"No dca Geometry for this track !!!"<<endl;continue;}
		...
		if(tofPid.beta()>0.0)
		{
			...
			if(tofbeta<1e-4 && dca.mag()<3.0)
			{
				...
				if(tof>0)tofbeta = L/(tof*(C_C_LIGHT/1.e9));
				else tofbeta = -999.0;
			}
		}
...
//		if(Float_t(nHitsFit)/Float_t(track->nHitsPoss())>1.02)continue;
		...
		if(tofbeta>0.0)hInvBetaP->Fill(p.mag()*charge,1.0/tofbeta);//beta vs. ragidity
...
//			}
	...
	for(unsigned int i=0;i<mDauVec1.size();i++)
	{
		...
		if(!dcaG1){cout<<"No dca Geometry for this track !!!"<<endl;continue;}
...
//		 }
     ...
     for(unsigned int j=0;j<mDauVec2.size();j++)
	 {
		 ...
		 if(!dcaG2){cout<<"No dca Geometry for this track !!!"<<endl;continue;}
