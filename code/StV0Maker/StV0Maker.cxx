...
StV0Maker::StV0Maker(StMuDstMaker* maker, const char* name) : StMaker(name) {
  ...
  mMuDstMaker =
      maker;  // Pass MuDst pointer to DstAnlysisMaker Class member functions
  ...
  mHistogramOutputFileName = "";  // Histogram Output File Name will be set
...
void StV0Maker::initHisto() {
  ...
  // vectors.
  // it is not useful. there are no need to operate all the histograms at the
  // same time.
...
Int_t StV0Maker::Make() {
  ...
  // gets reported as VtxPosition=(0,0,0).
  //	Skip events that do not have a primary vertex. the small value '1e-5' is
  // ok for hard coding.
  if ((fabs(muEvent->primaryVertexPosition().x()) < 1e-5) &&
      (fabs(muEvent->primaryVertexPosition().y()) < 1e-5) &&
      (fabs(muEvent->primaryVertexPosition().z()) < 1e-5))
    return kStOK;
  ...
  if (mBTofHeader) mvpdVz = mBTofHeader->vpdVz();
  ...
  hVertexZ->Fill(muEvent->primaryVertexPosition().z());  // Make histogram of
                                                         // the vertex Z
                                                         // distribution
  hSelectNRefMult->Fill(temprefMult);  // this is an ESSENTIAL histogram to
                                       // record the total number of events for
  ...
  while ((track = (StMuTrack*)GetTracks.Next())) {
    ...
    Double_t dau1Z = TMath::Log(
        dedx /
        dedx_dau1_th[index_for_p(abs(mCharge1) * p.mag())]);  // He3 charge = 2.
                                                              // build in mom
                                                              // p/q,should be
                                                              // corrected here!
    Double_t dau2Z = TMath::Log(
        dedx /
        dedx_dau2_th[index_for_p(abs(mCharge2) * p.mag())]);  // He3 charge = 2.
                                                              // build in mom
                                                              // p/q,should be
                                                              // corrected here!
