...
// helices.
// this is not a code for general purpose. use it when you understand the
// parameters
// inside the code. for specific, the current parameters are tuned for STAR TPC
// track.
...
// reconstruction.
// but minimization is essential for photon reconstruction. since adding
// minimization causes
// only minor lag in performance, we should keep it as default, even if for
// lambda reconstruction.
bool kMinimize = true;

// parameters to control the usage of minimization: for lambda recontruction
// 0.3, 0.3 is safe.
// for photon recontruction, 2, 2 is safe. By 'safe', I mean there is no signal
// loss compared to
// the sthelix dca method. For photons, this is essential to keep all signals
// (with the
// payment of performance), since we want to use same sign method to estimate
// the background.
// there will be systematic deviation if the signal loss (due to limited
// accuracy)
...
double closestDistance(const StPhysicalHelixD& helix1,
  ...
  if (d_p_pi >= (r_p + r_pi) ||
      d_p_pi <=
          fabs(r_p - r_pi)) {  // XZHU: it will be rare that two tracks with
                               // d_p_pi == r_p+r_pi come from a single particle
                               // decay. that means their momenta should be in
                               // the same direction or in opposite direction.
                               // it is normal for photon conversion to have two
                               // daughter electrons in the same direction. But
                               // it will be rare for two v0 decay daughters in
                               // the very opposite direction (that means the pt
                               // of v0 is 0). FIXME: please notice the hard cut
                               // 2.0cm above, you might change it for some
                               // purposes.
    fi_root1_p = 0.;
    fi_root2_p = 0.;
    if (d_p_pi >= (r_p + r_pi)) {
  ...
  fi_root1_p =
      fi_root1_p - floor(fi_root1_p / 2 / PI) * 2 *
                       PI;  // XZHU: try floor() here, instead of (int). since a
                            // wide range of period is tried below, the result
                            // should be the same. in fact, fi_root1_p should
                            // always be less than 0 (0,-2pi for 1 period),
                            // since we are looking backward for the position
                            // where the helix originates. ADDED: it is not so
                            // simple. we should keep the code as it is.
...
void dcaPToPi(float* fi_p, float* fi_pi, StTrackHelix* proton,
              StTrackHelix* pion, float* d_root, float* v0, float alfa) {
...
double closestDistance(const StThreeVectorF& xv0, const StThreeVectorF& pv0,
  ...
  xc = (x + r * sign * pyK / pbach.perp());  // get the x of the center of the
                                             // pion track //XZHU: why not
                                             // gtrack->Xc ?
  yc = (y - r * sign * pxK /
                pbach.perp());  // get the Y of the center of the pion track
  ...
  if (dmax > r) {  // no 2d crossing points but not too far either, then the
                   // newx and newy are the coordinates of the 2D closest point
                   // to the Lambda line but still on the circle. //XZHU: should
                   // be rare, since it needs the momenta of lambda and K is in
    ...
    dsave = sqrt((newzL - newz) * (newzL - newz) +
                 (dmax - r) * (dmax - r));  // XZHU: fixed! in this case, we can
                                            // not use the straight line
                                            // approximation below. because
                                            // these two lines always cross each
                                            // other at somewhere far far away.
                                            // therefore, you will have
                                            // unphysical '0' dca, some of them
                                            // will add up to the background.
    // tanK=-1;
    tanK = 0;
    ...
    float newz1 =
        z + sign0 * 2.0 * r * asin(da1 / 2.0) * pzK /
                sqrt(pxK * pxK + pyK * pyK);  // XZHU: only work in one period.
                                              // for sign0=1, it is weird to
                                              // check the orign point in the
                                              // forward direction
    ...
    float newz2 = z + sign0 * 2.0 * r * asin(da2 / 2.0) * pzK /
